                ...
                <tbody>
                    @foreach($filas as $fila)
                        <tr>
                            ...
                            <!-- BOTON EDITAR -->
                            <td>
                                <a href="{{ route('puntuaciones.edit', $fila->id) }}" class="inline-flex items-center hover:text-blue-600" aria-label="Editar puntuacion">
                            ...
                            <!-- BOTON BORRAR -->
                            <td>
                                <form onsubmit="event.preventDefault()" id="formulario{{$fila->id}}" action="{{ route('puntuaciones.destroy', $fila->id) }}" method="POST">
                                    @csrf
                                    @method("DELETE")
                            ...
                            <!-- BOTON VER -->
                            <td>
                                <a href="{{ route('puntuaciones.show', $fila->id) }}" class="text-white hover:text-blue-600 inline-flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" class="w-6 h-6">
